...
{
public:
	mouse_keyboard_game_controller_c() = default;
	...
	void associate_with_mouse(ACTION_ID action, INPUT_CONTROLLER_MOUSE_MOVE_MASK mouse_mask, bool activate_on_move = true);
private:
	...
	struct ACTION_DESC
	{
		...
		bool activate_on_move = false;
		INPUT_CONTROLLER_MOUSE_MOVE_MASK mouse_mask = MOUSE_MOVE_MASK_NOMOUSE;
		INT16 ctrl_pos = -1;
		INT16 ctrl_vel = 0;
	};
	...
	enum UPDATE_ACTION_PROC
	{
		UPDATE_ACTION_SET,
		UPDATE_ACTION_RESET,
		UPDATE_ACTION_MOVE_X,
		UPDATE_ACTION_MOVE_Y,
		UPDATE_ACTION_MOVE_WHEEL,
	...
	std::vector<ACTION_DESC*> mouse_actions;
